...
func (f *Tinkerpop) thingOrActionToTinkerpop(UUID strfmt.UUID, thing *models.Thing, typeOfObject string, addOrUpdate string) error {
	...
	if addOrUpdate == "add" {
		vertex.WriteString(`g.addV("` + thing.AtClass + `")`)
...
func (f *Tinkerpop) processSingleThing(result interface{}, thingNo int, thingResponse *models.ThingGetResponse) error {
	...
	for key, value := range result.([]interface{})[0].([]interface{})[thingNo].(map[string]interface{}) {
		...
		// add the properties to the edges. Note that the NrDollarCref is _not_ the Gremlin/Tinkerpop ID but the Weaviate UUID
