...
func (f *Janusgraph) createCrefObject(UUID strfmt.UUID, location string, refType connutils.RefType) *models.SingleRef {
...
func (f *Janusgraph) getSinglePropertyValue(haystack interface{}, needle string, no int) interface{} {
	// Loop over the Janusgraph results
	for k, v := range haystack.([]interface{})[0].([]interface{})[no].(map[string]interface{}) {
		// find properties if all properties are selected (i.e., no `.properties(...)` in the Janusgraph query)
		if k == "properties" {
...
func (f *Janusgraph) connectToKey(UUID strfmt.UUID, KeyUUID strfmt.UUID, typeToConnect string) error {
...
func (f *Janusgraph) keyUUIDFromEdge(edgeValue interface{}) (strfmt.UUID, error) {
